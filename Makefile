...
release: clean
	...
	@echo "GOPATH=${GOPATH}"
	@mkdir -p ${DIST_DIR}
	@CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -tags netgo -ldflags '-w -X main.BuildTime=${CUR_TIME} -X main.Version=${VERSION} -X main.GitHash=${GIT_COMMIT}' -o ${DIST_DIR}/${BIN_NAME}
